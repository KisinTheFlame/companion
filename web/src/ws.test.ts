...
describe("handleMessage: session_phase", () => {
  ...
  it("clears relaunchStatus when phase reaches a live state", () => {
    // After a successful auto-relaunch the CLI eventually emits a live phase
    // (ready/streaming/...). At that point the sidebar's "retrying N/M" hint
    // must drop, otherwise the UI keeps suggesting recovery is still happening.
    wsModule.connectSession("s1");
    fireMessage({ type: "session_init", session: makeSession("s1") });
    useStore.getState().setRelaunchStatus("s1", {
      attempt: 2,
      maxAttempts: 6,
      nextRetryAt: null,
      exhausted: false,
    ...
    expect(useStore.getState().relaunchStatus.get("s1")).toBeUndefined();
  });
});

// ===========================================================================
// handleMessage: relaunch_status
// ===========================================================================
describe("handleMessage: relaunch_status", () => {
  it("stores retry progress so the sidebar can render a countdown", () => {
    wsModule.connectSession("s1");
    fireMessage({ type: "session_init", session: makeSession("s1") });
    ...
    expect(useStore.getState().relaunchStatus.get("s1")).toEqual({
      attempt: 2,
      maxAttempts: 6,
      nextRetryAt: 1_700_000_000_000,
      exhausted: false,
  ...
  it("clears the entry when the orchestrator reports success", () => {
    // The orchestrator broadcasts attempt=0/exhausted=false/nextRetryAt=null
    // after a successful relaunch — that should drop the sidebar hint.
    wsModule.connectSession("s1");
    fireMessage({ type: "session_init", session: makeSession("s1") });
    useStore.getState().setRelaunchStatus("s1", {
      attempt: 1,
  ...
  it("keeps the entry with exhausted=true when the budget runs out", () => {
    // After MAX_AUTO_RELAUNCHES, the orchestrator broadcasts exhausted=true.
    // The sidebar relies on this to switch to the warning-tone hint.
    wsModule.connectSession("s1");
    fireMessage({ type: "session_init", session: makeSession("s1") });
