...
describe("SessionItem", () => {
  ...
  // --- Auto-relaunch retry hint ---

  it("shows the retry hint with a countdown while a relaunch is scheduled", () => {
    // While the orchestrator is between attempts, the sidebar surfaces
    // "{seconds}s 后重试 (n/m)" so the user can see auto-recovery is happening
    // instead of staring at a silent yellow spinner.
    vi.useFakeTimers();
    ...
    try {
      ...
      const hint = screen.getByTestId("session-retry-hint");
      expect(hint.textContent).toMatch(/7s 后重试 \(2\/6\)/);
    } finally {
      vi.useRealTimers();
    }
  });

  it("falls back to a non-countdown hint when nextRetryAt is missing", () => {
    // The retry message that fires *during* an in-flight attempt has no
    // future timestamp, so we should still show progress without a countdown.
    render(
      <SessionItem
        {...buildProps({
          session: makeSession({
    ...
    expect(screen.getByTestId("session-retry-hint").textContent).toBe("重试中 1/6");
  });

  it("shows the exhausted hint and uses the warning color", () => {
    // When automatic recovery gives up, the hint switches to a warning tone
    // so the user knows manual intervention is required.
    render(
      <SessionItem
        {...buildProps({
          session: makeSession({
    ...
    const hint = screen.getByTestId("session-retry-hint");
    expect(hint.textContent).toMatch(/重试已用尽（6\/6）/);
    expect(hint.className).toContain("text-cc-warning");
  });

  it("hides the retry hint for archived sessions", () => {
    // Archived sessions should not surface auto-relaunch chrome — the agent
      ...
      <SessionItem
        {...buildProps({
          ...
          session: makeSession({
            ...
            relaunchAttempt: 3,
            relaunchMaxAttempts: 6,
            relaunchNextRetryAt: Date.now() + 5_000,
            relaunchExhausted: false,
          }),
        })}
      />,
    );
    expect(screen.queryByTestId("session-retry-hint")).toBeNull();
  });
});
